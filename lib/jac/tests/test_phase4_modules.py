...
- src/jac/lsp-tools.ts → _lsp_helpers_toolchain.py
...
class TestParseCheckOutput:
    def test_gcc_format(self):
        from _lsp_helpers_toolchain import parse_check_output
        output = "main.jac:10:5: error: undefined variable 'x'"
        result = parse_check_output(output)
        assert len(result) == 1
        assert result[0]["file"] == "main.jac"
        assert result[0]["line"] == 10
        assert result[0]["column"] == 5
        assert result[0]["severity"] == "error"
        assert "undefined variable" in result[0]["message"]

    def test_multiple(self):
        from _lsp_helpers_toolchain import parse_check_output
        output = "a.jac:1:1: error: foo\nb.jac:2:2: warning: bar"
        result = parse_check_output(output)
        assert len(result) == 2
    ...
    def test_ansi_stripped(self):
        from _lsp_helpers_toolchain import parse_check_output
        output = "\x1b[31ma.jac:1:1: error: bad\x1b[0m"
        result = parse_check_output(output)
        assert len(result) == 1

    def test_fallback_with_default_file(self):
        from _lsp_helpers_toolchain import parse_check_output
        output = "something went wrong\nerror: cannot parse"
        result = parse_check_output(output, default_file="main.jac")
        assert len(result) == 1
        assert result[0]["file"] == "main.jac"
...
class TestExtractSymbol:
    def test_basic(self):
        from _lsp_helpers_toolchain import extract_symbol
        assert extract_symbol("hello world", 3) == "hello"

    def test_dotted(self):
        from _lsp_helpers_toolchain import extract_symbol
        assert extract_symbol("foo.bar.baz", 8) == "baz"

    def test_out_of_bounds(self):
        from _lsp_helpers_toolchain import extract_symbol
        assert extract_symbol("abc", 10) == ""

    def test_empty_line(self):
        from _lsp_helpers_toolchain import extract_symbol
        assert extract_symbol("", 0) == ""


class TestEscapeRegex:
    def test_special_chars(self):
        from _lsp_helpers_toolchain import escape_regex
        assert escape_regex("a.b*c+d") == r"a\.b\*c\+d"
...
class TestFormatLspDiagnostics:
    def test_empty(self):
        from _lsp_helpers_toolchain import format_lsp_diagnostics
        assert format_lsp_diagnostics([]) == "No diagnostics."

    def test_single(self):
        from _lsp_helpers_toolchain import format_lsp_diagnostics
        result = format_lsp_diagnostics([{
            "file": "a.jac", "line": 5, "column": 3, "severity": "error", "message": "bad"
        }])
        assert "a.jac:5:3 [error] bad" == result

    def test_with_code_and_source(self):
        from _lsp_helpers_toolchain import format_lsp_diagnostics
        result = format_lsp_diagnostics([{
            "file": "a.jac", "line": 1, "severity": "warning", "message": "x",
            "code": "W001", "source": "linter"
        }])
        assert "[W001]" in result
        assert "(linter)" in result


class TestFormatHoverInfo:
    def test_list_contents(self):
        from _lsp_helpers_toolchain import format_hover_info
        result = format_hover_info({"contents": ["line1", "line2"]})
        assert result == "line1\nline2"

    def test_string_contents(self):
        from _lsp_helpers_toolchain import format_hover_info
        result = format_hover_info({"contents": "just text"})
        assert result == "just text"


class TestFormatLocations:
    def test_empty(self):
        from _lsp_helpers_toolchain import format_locations
        assert format_locations([]) == "No results found."

    def test_basic(self):
        from _lsp_helpers_toolchain import format_locations
        result = format_locations([
            {"file": "a.jac", "line": 10, "text": "can my_func()"},
            {"file": "b.jac", "line": 20},
        ])
        assert "a.jac:10: can my_func()" in result
        assert "b.jac:20" in result

    def test_truncation(self):
        from _lsp_helpers_toolchain import format_locations
        long_text = "x" * 200
        result = format_locations([{"file": "a.jac", "line": 1, "text": long_text}])
        assert "..." in result

    def test_max_20(self):
        from _lsp_helpers_toolchain import format_locations
        locs = [{"file": f"f{i}.jac", "line": i} for i in range(30)]
