...
| M5 (slice 2) | 2026-08-24 | `526df80`+`(this)` | PASS - CORPUS ALL GREEN (6/6 cases via jac run driver). Frozen oracle at ui/testing/legacy_layout_oracle.jac (verbatim snapshot of measure/arrange at 68eca8d). Dual-run corpus ui/layout_corpus.test.jac: shell x 7-viewport matrix, wrapped+scroll clamp, content matrix, horizontal row, nested 3-grow LR divergence, overlay placement/clip. Quantizer gained the plan-8.3 branch: <3 flexible targets bias leftover to last child (legacy parity); one M4 expectation updated accordingly ([3,4,3] -> [3,3,4] for fixed+two-flexible). Engine overlay clip now == rect (strict parity; renderer bounds its own paint). jac check clean on all touched modules; quantize suite 19/19 green before the daemon crash. Corpus execution blocked by jaclang embedded-postgres breakage post-crash (each run mints an unbound socket dir; "postgres not ready after 60s" on every test invocation, including previously-green suites). Corpus fixes found by the run: (1) engine naturals were content-only - added _structural_naturals (flow=sum, cross=max, FIXED=preferred) mirroring legacy _measure_node; leaves wrap at hint width (viewport cols phase H, quantized widths phase V); (2) empty clip now uses the Rect(0,0,0,0) sentinel exactly like oracle _clip_intersect; (3) quantizer <3-flexible branch guarded for all-fixed partitions; (4) STRETCH+FIXED cross children need explicit START to match legacy top-left placement. Regression: full 367-test layout batch (engine/compile/quantize/state/screen) green. HUMAN HANDOFF (jaclang): post-crash the pytest plugin fails collection with ModuleNotFoundError for any NEWLY created test file while pre-crash files and fresh jac check/run compiles work; corpus therefore executes via ui/corpus_driver.jac (jac run). Also pgembed socket-dir reuse after daemon crash still needs upstream attention. |
...
| M5 | | | |
| M6 | 2026-08-23 | (this commit) | PASS. Atomic cutover: legacy engine hard-deleted (`layout_state.jac(+test)`, `layout_engine.jac`, `layout_compute.test.jac`, `layout_corpus.{jac,test}` + cases, `mini_probe.test.jac`, `corpus_driver.jac`, `testing/legacy_layout_oracle.jac`; −4,087 LOC). Permanent seam finalized: `ui/layout_types.jac` is the leaf (value types + spec helpers + registry lifecycle incl. create/set_spec/add_relation/set_content/clear_node/frame accessors); `ui/layout.jac` re-exports it and hosts only compute_layout. All consumers migrated: renderer (LayoutTables→LayoutState, get_entry→frame_entry_of, measure_and_arrange→compute_layout + private _snapshot_rects replacement; mark_*_dirty kept as honest pending markers), gates (specs via fill/fixed/grow_axis), screen/tui/demos/n1_acceptance (set_spec-based; tui keeps header=2/prompt=3 parity overrides after configure_main_shell_layout), jac.toml pins pruned. Same-stem suites merged into ui/layout.test.jac (90/90); layout_compile.test.jac repointed to leaf imports (43/43); quantize 19/19; screen 75/75. Root-cause lesson recorded: a stale same-stem annex importing deleted modules poisons EVERY importer of its base module with misleading partial-init ImportErrors — annexes must import leaves only. Renderer/gates/demo/n1 gates verified by jac check (all 13 touched files 0 errors) + subagent diff review; remaining full-suite runs deferred (host resource pressure). |
| M6 | | | |
