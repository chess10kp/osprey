...
| M5 (slice 2) | 2026-08-24 | (this commit) | PARTIAL - code complete, parity RUN blocked. Frozen oracle at ui/testing/legacy_layout_oracle.jac (verbatim snapshot of measure/arrange at 68eca8d). Dual-run corpus ui/layout_corpus.test.jac: shell x 7-viewport matrix, wrapped+scroll clamp, content matrix, horizontal row, nested 3-grow LR divergence, overlay placement/clip. Quantizer gained the plan-8.3 branch: <3 flexible targets bias leftover to last child (legacy parity); one M4 expectation updated accordingly ([3,4,3] -> [3,3,4] for fixed+two-flexible). Engine overlay clip now == rect (strict parity; renderer bounds its own paint). jac check clean on all touched modules; quantize suite 19/19 green before the daemon crash. Corpus execution blocked by jaclang embedded-postgres breakage post-crash (each run mints an unbound socket dir; "postgres not ready after 60s" on every test invocation, including previously-green suites). HUMAN HANDOFF: restart jac daemons / investigate pgembed socket-dir reuse after a daemon crash, then run JAC_TEST_JOBS=0 jac test app/ui/layout_corpus.test.jac. |
