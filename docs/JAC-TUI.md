...
Canonical policy: [`AGENTS.md`](../../AGENTS.md) § agent-next migration notes.
...
shell.cl.jac  ──jac tui──▶  .jac/tui/module.mjs + runner.mjs
                    ...
                    @jac/pi hooks ◀─┘  (jac_pi_runtime_shim.mjs — emitted by jac-ink, human-owned)
                                    │
                    agent-next/dist/  (createNextAgent — jackal repo)
```
...
| **Stop hardcoding Jackal paths** | Current shim tries `../../../../jackal/agent-next/dist/index.js` — should be env/config only | jac-ink |
...
|-------|------|---------------------|
| **2** | Emit `jac_pi_adapter.mjs` — store, event bridge, hooks | Partially — Jackal already has this in `agent-next/src/` |
...
1. **Embedded adapter mode** (Jackal) — external `createNextAgent()`, lightweight runtime
2. **Full Pi SDK mode** (generic jac-ink) — `createAgentSession()`, extensions, skills
...
3. **Dual-mode architecture** — `pi-agent-core`-only (Jackal) vs `pi-coding-agent` (generic jac-ink)
